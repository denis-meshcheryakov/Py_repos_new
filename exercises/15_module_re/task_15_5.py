...
Функция должна обрабатывать вывод команды show cdp neighbors и
генерировать на основании вывода команды описание для интерфейсов.
...
def generate_description_from_cdp(cdp_file):
    regex = re.compile(r'(?P<device>\w+)  +(?P<loc_intf>\S+ \S+)'
